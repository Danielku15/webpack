...
/** @typedef {import("./ModuleDependency")} ModuleDependency */
...
const DEFAULT_SYNTAX = [
	...
	"*context.paintWorklet.addModule<>",
	"*context.layoutWorklet.addModule<>",
	"*context.animationWorklet.addModule<>",
	...
	apply(compiler) {
		...
		compiler.hooks.thisCompilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				...
				/** @typedef { (block:AsyncDependenciesBlock, expr:CallExpression, url:string, urlRange: Range, publicPath: string) => void } CreateDependencies */
				/**
				...
				const parserPlugin = (parser, parserOptions) => {
					 ...
					 * @param {CreateDependencies} createDependencies the dependency factory
					 * @returns {boolean | void} true when handled
					 */
					const handleNewWorker = (expr, createDependencies) => {
					...
					const createWorkerDependencies = (block, expr, url, urlRange) => {
					...
					/** @type {CreateDependencies} */
					const createWorkletDependencies = (block, expr, url) => {
						// TODO: is there a better way that we can write
						// specific expressions instead of overwrite parts with strings?

						// before the worklet call we inject the custom function
						// and we make the original expression the argument to it.
						const workletBootstrap = new WorkletDependency(
							url,
							[expr.range[0], expr.range[0]],
							{
								publicPath: this._workerPublicPath,
								getChunkFileName(/**@type {Chunk} */ chunk) {
						...
						workletBootstrap.loc = /** @type {DependencyLocation} */ (expr.loc);
						block.addDependency(workletBootstrap);

						// clear out .addModule(url)

						// should always be the case as we have <something>.addModule() which is a MemberExpression call
						const member = /** @type {MemberExpression} */ (expr.callee);

						const endOfCall = new ConstDependency(")", [
							member.object.range[1],
							expr.range[1]
						]);
						endOfCall.loc = /** @type {DependencyLocation} */ (expr.loc);
						block.addDependency(endOfCall);
					};
					...
					const processItem = item => {
						if (
							...
							const createDependencies = item.endsWith("<>")
								? createWorkletDependencies
							...
							parser.hooks.callMemberChain
								...
								.tap(PLUGIN_NAME, (expression, members) => {
									...
									return handleNewWorker(expression, createDependencies);
								});
						} else if (item.endsWith("()")) {
							parser.hooks.call
								.for(item.slice(0, -2))
								.tap(PLUGIN_NAME, expr =>
									handleNewWorker(expr, createWorkerDependencies)
								);
						} else {
							...
							if (match) {
								...
								(call ? parser.hooks.call : parser.hooks.new)
									...
									.tap(PLUGIN_NAME, expr => {
										...
										return handleNewWorker(expr, createWorkerDependencies);
									});
							} else {
								parser.hooks.new
									.for(item)
