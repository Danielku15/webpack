...
const GetWorkerUrlRuntimeModule = require("./runtime/GetWorkerUrlRuntimeModule");
...
class RuntimePlugin {
	...
	apply(compiler) {
		compiler.hooks.compilation.tap("RuntimePlugin", compilation => {
			...
			compilation.hooks.runtimeRequirementInTree
				.for(RuntimeGlobals.getWorkerUrl)
				.tap("RuntimePlugin", (chunk, set) => {
					if (
						typeof compilation.outputOptions.chunkFilename === "string" &&
						/\[(full)?hash(:\d+)?\]/.test(
					...
					compilation.addRuntimeModule(
						chunk,
						new GetWorkerUrlRuntimeModule(RuntimeGlobals.getWorkerUrl)
					);
					return true;
				});
			compilation.hooks.runtimeRequirementInTree
